...
// get di un utente
comment.get("/video/:videoId/comment", verifyToken, async (req, res) => {
  const videoId = req.params.videoId;

  try {
    const commentsForPost = await CommentModel.find({ videoId: videoId })
      .populate("author")
      .exec();
...
comment.get(
  ...
  async (req, res) => {
    ...
    try {
      const specificComment = await CommentModel.findOne({
        _id: commentId,
        videoId: videoId,
      })
        .populate("author")
